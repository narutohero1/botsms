...
token = "OTQ5NTE1MDU2ODg5MDk4MjQw.YiLezg.JyxkvofK4YhGh9FWlVboQqhuXFE"
...
async def sms(ctx, phone, amount:int):
    ...
    if (amount < 501):
      ...
      embes = discord.Embed(title="#ꜱᴍꜱ-ᴀᴛᴛᴀᴄᴋ", description="Bot by : sms",color=0xff4612)
      embes.add_field(name="กำลังยิงไปที่เบอร์",value=phone)
      ima = "https://media.giphy.com/media/2a5eqgUwH70CWn0H7v/giphy.gif"
      ...
      BBot(phone,amount)
    	...
    	await ctx.channel.send("ยิงเกิน 500 ผมยิงไม่ไหว🤕🤒")
...
@bot.command()
async def help(ctx):
	emBed = discord.Embed(title="Bot BY: sms",description="วิธิใช้",color=0xff4612)
